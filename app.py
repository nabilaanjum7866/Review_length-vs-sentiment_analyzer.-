...
with st.sidebar.expander("ℹ️ About App"):
    st.info("This app predicts sentiment using ML + rule-based logic with Neutral detection.")
...
with tab1:
    ...
    if st.button("🚀 Analyze"):
        if review.strip() == "":
            ...
            rule = rule_based_fix(review)

            if rule:
                prediction = rule

                # ✅ FIXED CONFIDENCE (NO RANDOM)
            ...
            if prediction == "positive":
                st.markdown(f'<div class="result-box positive">Positive ({confidence}%) 😊</div>', unsafe_allow_html=True)
            elif prediction == "negative":
                st.markdown(f'<div class="result-box negative">Negative ({confidence}%) 😠</div>', unsafe_allow_html=True)
            else:
                st.markdown(f'<div class="result-box neutral">Neutral ({confidence}%) 😐</div>', unsafe_allow_html=True)
